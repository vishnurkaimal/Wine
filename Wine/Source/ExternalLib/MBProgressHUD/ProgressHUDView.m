...
//  MiGym
//
//  Created by Sujan Kanna on 2/25/11.
//  Copyright 2011 Solstice Consulting. All rights reserved.
//
...
- (void)show:(BOOL)animated {
	...
	if (thumbImageView && [thumbImageView superview]) {
...
- (void)hide:(BOOL)animated {
	useAnimation = animated;
	if (!thumbImageView) {
		thumbImageView = [[UIImageView alloc] initWithFrame:indicator.frame];
		thumbImageView.image = [UIImage imageNamed:@"thumbs_up_icon.png"];
	}
...
- (void)drawRect:(CGRect)rrect {
	...
	CGPathMoveToPoint(path, NULL, CGRectGetMinX(boxRect) + radius, CGRectGetMinY(boxRect));
	CGPathAddArc(path, NULL, CGRectGetMaxX(boxRect) - radius, CGRectGetMinY(boxRect) + radius, radius, 3 * M_PI / 2, 0, 0);
	CGPathAddArc(path, NULL, CGRectGetMaxX(boxRect) - radius, CGRectGetMaxY(boxRect) - radius, radius, 0, M_PI / 2, 0);
	CGPathAddArc(path, NULL, CGRectGetMinX(boxRect) + radius, CGRectGetMaxY(boxRect) - radius, radius, M_PI / 2, M_PI, 0);
	CGPathAddArc(path, NULL, CGRectGetMinX(boxRect) + radius, CGRectGetMinY(boxRect) + radius, radius, M_PI, 3 * M_PI / 2, 0);
	
	CGColorSpaceRef rgb = CGColorSpaceCreateDeviceRGB();
	CGFloat colors[] =
	{
		150/255.0, 150/255.0, 170/255.0, 1.0,
		70/255.0, 70/255.0, 50/255.0, 1.0,
		30/255.0, 30/255.0, 50/255.0, 1.0
	};
	
	CGGradientRef gradientFill = CGGradientCreateWithColorComponents(rgb, colors, NULL, sizeof(colors)/(sizeof(colors[0])*4));
	...
	CGContextDrawLinearGradient(context, gradientFill, myStartPoint, myEndPoint, kCGGradientDrawsAfterEndLocation);
